...
function NuN_OnLoad(self)
	...
	--	hooksecurefunc("QuestLog_Update", NuNNew_QuestLog_Update);		@todo xaemyl: update and restore this code
...
function NuN_OnEvent(self,event,...)
	...
	elseif ( event == "ADDON_LOADED" ) then
		...
		elseif ( arg1 == "Blizzard_AuctionUI" ) then
			NuNQuickNote.RegisterModifier_AuctionFrame();
	...
	if (newTxt ~= locals.originalText) then
		...
		if (locals.isTitle) then
			if ( NuNEditDetail_CheckButton:GetChecked() ) then
				locals.headingNumber = locals.pHead..locals.bttnNumb;
				locals.headingName = local_player.currentNote.unit..locals.headingNumber;
...
function NuN_AH_BrowseButton_OnClick(self,btn,down)
	local bResult = false;
	if NuNHooks.originalOnClick[self:GetID()] then
		bResult = NuNHooks.originalOnClick[self:GetID()](self, btn, down);
	end
	...
	if ( ( NuNSettings[local_player.realmName].modifier == "on" ) and ( IsNuNModifierKeyDown(down) ) ) then
		...
		bResult = NuNQuickNote.ProcessHyperlink(itmLink);
	end
...
function NuNQuickNote.RegisterModifier_AuctionFrame(mouseButtonName)
	...
	for buttonIdx = 1, 8 do
		for typeIdx = 1, getn(buttonType) do
			...
			bttn = _G[frameName];
			if bttn then
				if mouseButtonName then
					bttn:RegisterForClicks(mouseButtonName .. "Up");
				else
					bttn:RegisterForClicks("AnyUp");
				end
				
				local func = bttn:GetScript("OnClick");
				if func and NuNHooks.originalOnClick and NuNHooks.originalOnClick[bttn:GetID()] == nil then
					NuNHooks.originalOnClick[bttn:GetID()] = func;
...
function NuNF.RegisterModifier_TrainerFrame(buttonName)
	...
	for i = 1, MAX_NUM_TALENTS do
		...
		if ( button ) then
			...
			button:RegisterForClicks(buttonName .. "Up");
			if func ~= NuNHooks.originalOnClick[button:GetID()] then
				button:SetScript("OnClick", function(frame, mouseButton, isMouseButtonDown)
					NuNHooks.originalOnClick[button:GetID()](frame, mouseButton, isMouseButtonDown);
				end);
				NuNHooks.originalOnClick[button:GetID()] = func;
			end
...
function NuNF.RegisterModifier_Main(buttonName)
	for i = 1, 10 do _G["QuestInfoItem" .. i]:RegisterForClicks(buttonName .. "Up"); end
	for i = 1, 12 do _G["CompanionButton" .. i]:RegisterForClicks(buttonName .. "Up"); end
	--CharacterAmmoSlot:RegisterForClicks(buttonName .. "Up");
...
function NotesUNeed_RegisterFramesForClicks()
...
-- debug output
		if locals.NuNDebug and locals.debugging_msg_hooks then
			nun_msgf("NuN_HandleModifiedItemClick - general_note_open:%s    player_note_open:%s     original_link:%s    bResult:%s   mouseButton:%s",
				tostring(NuNGNoteFrame:IsVisible()), tostring(NuNFrame:IsVisible()), itemLink, tostring(bResult), tostring(mouseButton));
		end
	end
